...
class campaignsController extends Controller
{
    ...
    public function AjaxDT(Request $request)
    {
        if (request()->ajax()) {
            ...
            return DataTables::of($campaigns)
                ...
                })->rawColumns(['actions', 'adminName', 'campName'])->make(true);
        }
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($request->quantity > $stock) {
            ...
            DB::table('givings')->update(['quantity' => $stock - $request->quantity]);
    ...
    public function edit($id)
    {
        ...
        return view('dashboard.campaigns.edit', compact('campaign', 'categories', 'admins'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        $campaign = Campaign::findOrFail($id);
        $campaign->update($request->only('title', 'description', 'category_id', 'admin_id'));

        return response()->json(['status' => 1, 'msg' => 'تم تحديث الحملة بنجاح']);
    }
